...
$(OBJ_CHIP_RTL_V): $(OBJ_CHIP_RTL_CHISEL_V) $(OBJ_CORE_RTL_V)
	cat $^ > $@
...
$(OBJ_CHIP_RTL_CHISEL): $(OBJ_CHIP_RTL_DECOUPLED_JSON) $(CHIP_GENERATOR_ADDON)/tools/generate-asic-top
	mkdir -p $(dir $@)
	$(CHIP_GENERATOR_ADDON)/tools/generate-asic-top --core-top $(CORE_TOP) --chip-top $(CHIP_TOP) --syn-top $(CHIP_TOP_FOR_SYNTHESIS) --decoupled $(filter %.decoupled.json,$^) --output $@

$(OBJ_CHIP_RTL_DECOUPLED_JSON): $(CMD_PCAD_INFER_DECOUPLED) $(OBJ_CORE_RTL_V)
	mkdir -p $(dir $@)
...
$(OBJ_CHIP_DIR)/csrc/%.cc: $(OBJ_CORE_RTL_C) $(patsubst %,$(OBJ_CHIP_DIR)/csrc/%,$(notdir $(wildcard $(patsubst %,%/*.h,$(sort $(dir $(OBJ_CORE_RTL_C)))))))
	mkdir -p $(dir $@)
	cat $(filter %/$(notdir $@),$^) | sed 's/$(CORE_TOP)/$(CHIP_TOP)/g' > $@

# So this is a huge hack: essentially what happens is that loads of 
.SECONDARY: $(patsubst %,$(OBJ_CHIP_DIR)/csrc/%,$(notdir $(wildcard $(patsubst %,%/*.h,$(sort $(dir $(OBJ_CORE_RTL_C)))))))
$(OBJ_CHIP_DIR)/csrc/%.h: $(wildcard $(patsubst %,%/*.h,$(sort $(dir $(OBJ_CORE_RTL_C)))))
	mkdir -p $(dir $@)
	cp -f $(filter %/$(notdir $@),$^) $@
