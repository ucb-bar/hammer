...
class HammerSubmitCommand:
    ...
    def get(tool_namespace: str, database: HammerDatabase) -> "HammerSubmitCommand":
        ...
        max_output_lines  = db.get_setting(ns+".max_output_lines")
        max_error_lines   = db.get_setting(ns+".max_error_lines")
        ...
        cmd.max_output_lines  = max_output_lines
        cmd.max_error_lines   = max_error_lines
    ...
    def submit(self, args: List[str], env: Dict[str, str],
        ...
        proc = subprocess.Popen(self.get_cmd_array(),
            shell=False,
        ...
        output_lines = []
        log_file = "{}/{}.log".format(cwd, prog_tag)
        ...
        with open(log_file, "w") as f:
            def print_to_all(line):
                ...
                if (self.max_lines is not None) and \
                        (len(output_lines) <= self.max_lines):
        ...
        for line in output_lines:
            if (self.max_errors is not None) and \
                    (len(error_lines) > self.max_errors):
...
class HammerLocalSubmitCommand(HammerSubmitCommand):
    ...
    def get_cmd_array(self, tag:str, args: List[str], env: Dict[str, str],
        ...
        return [write_run_script(tag, args, cwd)]
...
class HammerLSFSubmitCommand(HammerSubmitCommand):
    ...
    def get_cmd_array(self, tag:str, args: List[str], env: Dict[str, str],
        ...
        return self.bsub_args() + [write_run_script(tag, args, cwd)]
