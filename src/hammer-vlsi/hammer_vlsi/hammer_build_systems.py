...
def build_makefile(driver: HammerDriver, append_error_func: Callable[[str], None]) -> dict:
    ...
    make_text = textwrap.dedent("""
        ...
        drc{suffix}: {drc_out}
        lvs{suffix}: {lvs_out}
        static-verification{suffix}: {static-verification_out}
        ...
        {static-verification_out}: {syn_deps} $(HAMMER_POWER_DEPENDENCIES)
        \t$(HAMMER_EXEC) {env_confs} {p_static-verification_in} $(HAMMER_EXTRA_ARGS) --obj_dir {obj_dir} static-verification{suffix}
        ...
        .PHONY: redo-sim-rtl{suffix} redo-syn{suffix} redo-power{suffix} redo-sim-syn{suffix} redo-par{suffix} redo-sim-par{suffix} redo-power-par{suffix} redo-drc{suffix} redo-lvs{suffix}
    ...
    if not dependency_graph:
        ...
        static-verification_run_dir = os.path.join(obj_dir, "static-verification-rundir")
        ...
        p_static-verification_in = proj_confs
        static-verification_out = os.path.join(power_run_dir, "static-verification-output-full.json")
        ...
        output += make_text.format(suffix="", mod=top_module, env_confs=env_confs, obj_dir=obj_dir, syn_deps=syn_deps,
            ...
            p_static-verification_in=p_static-verification_in, static-verification_out=static-verification_out, static-verification_run_dir=static-verification_run_dir,
        ...
        for node, edges in dependency_graph.items():
            ...
            static-verification_run_dir = os.path.join(obj_dir, "static-verification-" + node)
            ...
            power_out = os.path.join(power_run_dir, "power-output-full.json")
            p_static-verification_in = proj_confs
            static-verification_out = os.path.join(static-verification_run_dir, "static-verification-output-full.json")
