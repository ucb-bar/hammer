...
class Metal(BaseModel):
    ...
    pitch: The minimum cross-mask pitch for this layer (NOT same-mask pitch
	   ...
	   To route denser wires on chip, multiple masks are required. During fabrication, 
	   the masks are applied separately with some spatial offsets to achieve denser line patterning.
	   [multiple-patterned](https://en.wikipedia.org/wiki/Multiple_patterning)
