...
class YosysSynth(HammerSynthesisTool, OpenROADTool, TCLTool):
    ...
    def syn_generic(self) -> bool:
        ...
        elif self._database.has_setting("synthesis.yosys.latch_map_resource"):
            path = self.get_setting("synthesis.yosys.latch_map_resource")
            package = path[0]
            resource = path[1]
            text = importlib.resources.files(package).joinpath(resource).read_text()
            latch_map_file = (Path(self.technology.cache_dir) / resource)
            latch_map_file.write_text(text)
            latch_map = f"techmap -map {str(latch_map_file)}"
